...
if (_useAceAction) then {
	_statement = {
		...
		private _action = ["TpMenu", LSTRING(TeleportMenu),"\a3\3den\data\cfg3den\history\changeattributes_ca.paa",{ [_target] call FUNC(openTeleportMenu) }, _menuOpenConditio] call ace_interact_menu_fnc_createAction;
		[_obj, 0, _actionPath, _action] call ace_interact_menu_fnc_addActionToObject;
	};
} else {
	_statement = {
		params ["_obj", "_menuOpenConditio"];
		_obj addAction [LSTRING(TeleportMenuVanilla), { [_this select 0] call FUNC(openTeleportMenu) }, [], 10, true, true, "", _menuOpenConditio, 10]
	};	
};
